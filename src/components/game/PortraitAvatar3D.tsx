...
/**
 * Photo-driven 3D model: maps the user's portrait onto a faceted holographic
 * crystal/bust with rotating displacement, scan-line shader, particle aura,
 * and orbital tech glyphs. Picks accent color from a palette switcher.
 */
function HoloBust({ accent }: { accent: string }) {
  ...
  useFrame(({ clock, pointer }) => {
    ...
    if (visor.current) {
      ...
      visor.current.position.y = 0.05 + Math.sin(t * 1.5) * 0.4; // scanning bar
    }
  ...
  return (
    <group ref={group} position={[0, -0.2, 0]}>
      {/* HOLO CARD with portrait */}
      <mesh ref={card} position={[0, 0.1, 0]}>
        <planeGeometry args={[2.1, 2.6, 24, 32]} />
      ...
      {/* corner brackets */}
      {([
        [-1.05, 1.3], [1.05, 1.3], [-1.05, -1.1], [1.05, -1.1],
        ...
        return (
          <Float key={l} speed={2} floatIntensity={0.6}>
            <Html position={[Math.cos(a) * r, Math.sin(a * 1.3) * 0.6, Math.sin(a) * r]} center>
              <div className="font-mono text-[10px] px-1.5 py-0.5 border" style={{ color: accent, borderColor: accent, background: "rgba(8,10,22,0.6)" }}>{l}</div>
            </Html>
          </Float>
        );
      })}
...
export function PortraitAvatar3D() {
  ...
  return (
    <div className="relative corner-frame box-glow bg-card/60 backdrop-blur-md h-[460px] overflow-hidden">
      ...
      <Canvas camera={{ position: [0, 0.2, 4.4], fov: 50 }}>
        <Suspense fallback={null}>
          ...
          <Stars radius={40} depth={20} count={1400} factor={2.5} fade speed={1} />
          <HoloBust accent={accent} />
      ...
      <div className="absolute top-3 left-3 font-mono text-[10px] text-primary/80">
        ⌬ HOLO.PROJECTION — AYUSH.MESH v3.0
      </div>
